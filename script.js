...
class Game {
  ...
  handleServerMessage(data) {
    switch (data.type) {
      ...
      case 'projectileCreated':
        this.projectiles.push(new Projectile(data.projectile.x, data.projectile.y, data.projectile.directionX, data.projectile.directionY, data.projectile.playerId, data.projectile.radius, data.projectile.speed, data.projectile.id));
        break;
      case 'hpUpdate':
  ...
  setupTouchControls() {
    ...
    document.addEventListener('touchmove', e => {
      for (let touch of e.touches) {
        if (touch.identifier === moveTouchId && this.joystick.active) {
          this.joystick.currentX = touch.clientX;
          this.joystick.currentY = touch.clientY;
        }
        if (touch.identifier === aimTouchId && this.player) {
          const dx = touch.clientX - this.player.x;
          const dy = touch.clientY - this.player.y;
  ...
  update() {
    ...
    if (this.isMobile && this.joystick.active) {
      ...
      if (len > deadZone) {
        const normX = dx / len;
        const normY = dy / len;
        const speed = Math.min((len - deadZone) * 0.12, 8);
        this.player.move(normX * speed, normY * speed);
        this.player.setDirection(normX, normY);
      }
    }
  ...
  playerHit() {
    this.killCounts.set(this.playerNetId, 0);
    this.isRunning = false;
...
class Player {
  ...
  move(dx, dy) {
    ...
    const g = window.game;
    if (g) {
      this.x = Math.max(this.radius, Math.min(g.mapWidth - this.radius, this.x));
      this.y = Math.max(this.radius, Math.min(g.mapHeight - this.radius, this.y));
    }
  }
...
// ------------------ 子彈 ------------------
class Projectile {
  constructor(x, y, dx, dy, playerId, radius = 5, speed = 10, id = null) {
    this.x = x; this.y = y;
    this.directionX = dx; this.directionY = dy;
    this.speed = speed; this.radius = radius;
    this.color = '#e74c3c'; this.playerId = playerId; this.id = id;
  }
